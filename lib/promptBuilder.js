...
export function adaptPromptForFramework(basePrompt, framework, colors = [], typography = {}, vibe = [], category = 'SaaS') {
  ...
  const vibeList = vibe && vibe.length > 0 ? vibe.map(v => `#${v}`).join(', ') : '#Full-Page Scroll, #Sticky Navbar, #Bento Grid, #High Contrast';

  const designTokensHeader = `
=== MASTER DESIGN SYSTEM TOKENS ===
- Background Canvas: ${bgHex} (${colors[0]?.name || 'Background'})
...
- Text & Headlines: ${textHex} (${colors[4]?.name || 'Text'})
